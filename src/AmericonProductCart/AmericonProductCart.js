...
const AmericonProductCart = ({ americanprod, counts, onCountChange, onRemoveProduct }) => {
  const handleIncrease = (index) => {
    onCountChange(index, counts[index] + 1);
  }; 
  ...
  return (
    <>
      {americanprod.map((americanproduct, index) => {
        const rate = parseFloat(americanproduct.rate.replace('$', ''));
        const total = rate * counts[index];
        ...
        return (
          <div key={index} className="americon-prod-cart">
            ...
            {/* TODO: Add a button to delete the product */}
            <button onClick={() => onRemoveProduct(index)}>Delete</button>
      })}
    </>
  );
};
